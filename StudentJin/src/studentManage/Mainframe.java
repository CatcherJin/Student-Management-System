...
public class Mainframe extends JFrame implements ActionListener {
	...
	static JRadioButton radioButton4 = new JRadioButton("宿管");
	...
	public Mainframe() {
		...
		radioButton1.setBounds(30, 140, 80, 20);
		radioButton2.setBounds(110, 140, 80, 20);
		radioButton3.setBounds(190, 140, 80, 20);
		radioButton4.setBounds(270, 140, 80, 20);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (radioButton3.isSelected())// 选中教师登录
		{
			try {
				Class.forName("com.mysql.jdbc.Driver");
				//Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
			...
			try {
				...
				Statement stmt = con.createStatement();
				//在数据库中验证用户名和密码
				ResultSet rs = stmt.executeQuery("select * from teacher where tId= '"
								+ textField1.getText() + "' and tpw= '"
								+ password.getText() + "'");
		...
		if (radioButton4.isSelected())// 选中管理员
		{
			//验证用户名和密码是不是suguan和123
			if (textField1.getText().equals("suguan")
					&& password.getText().equals("123") && textField3.getText().equals(num)) {
				try {
					Suguan b = new Suguan();
				} catch (Exception e1) {
